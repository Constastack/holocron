...
HELP_TEXT_PLAYER = (
    ...
    "`/deck-pridat` — uloží ti deck pro rychlejší hlášení výsledků\n"
    "`/moje-decky` — zobrazí tvé uložené decky\n"
    "`/deck-smazat` — smaže uložený deck\n"
...
HELP_TEXT_ORGANIZER = (
    ...
    "`/top-cut-nastavit` — nastaví kanál pro oznámení Top Cutu (pavouk, postupy, vítěz)\n"
...
async def setup_live_season_cmd(interaction: discord.Interaction):
    ...
    await _delete_old_live_panel(interaction.client, "season_panel_channel_id", "season_panel_message_id")
    try:
